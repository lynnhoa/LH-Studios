...
export default function A4Document({
  ...
  // ── Mini helpers ──────────────────────────────────────────
  const MRow = ({ lb, v }: any) => (
    <div style={PDF.DETAILS.METADATA.ROW}>
      <span style={PDF.DETAILS.METADATA.ROW_LABEL}>{lb}</span>
      <span style={PDF.DETAILS.METADATA.ROW_VALUE}>{v}</span>
    </div>
  );
